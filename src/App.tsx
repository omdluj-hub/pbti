...
const QUESTIONS: Question[] = [
  {
    id: 1,
    dimension: 'IE',
    text: "평소 피부가 뒤집어지는 가장 큰 원인은 무엇인가요?",
    options: [
  ...
  },
  {
    id: 3,
    dimension: 'IE',
    text: "피부 컨디션이 가장 나빠 보일 때는 언제인가요?",
    options: [
  ...
  },
  {
    id: 5,
    dimension: 'SN',
    text: "세안 후 아무것도 바르지 않았을 때, 내 피부 상태는?",
    options: [
  ...
  },
  {
    id: 6,
    dimension: 'SN',
    text: "메이크업이나 선크림을 바르고 4~5시간이 지났을 때?",
    options: [
  ...
  },
  {
    id: 11,
    dimension: 'TF',
    text: "병원이나 관리실을 방문할 때 나의 주된 목적은?",
    options: [
  ...
  },
  {
    id: 12,
    dimension: 'TF',
    text: "새로운 화장품이나 시술을 결정할 때 가장 중요한 기준은?",
    options: [
  ...
  },
  {
    id: 13,
    dimension: 'PJ',
    text: "컨디션이 정말 안 좋은 날, 주변 사람들의 반응은?",
    options: [
  ...
  },
  {
    id: 15,
    dimension: 'PJ',
    text: "피부에 문제가 생겼을 때, 내가 느끼는 당혹감은?",
    options: [
...
const TYPE_INFO: Record<string, { title: string, desc: string, care: string, emoji: string }> = {
  ISTP: { title: "속깊은 은둔 고수", emoji: "🏠", desc: "겉으로는 평온해 보이지만, 내부 요인에 의해 유분이 올라오는 타입입니다.", care: "과도한 유분을 조절하면서 피부속 열을 내리는 진정팩을 추천해요." },
  ISTJ: { title: "예민한 완벽주의자", emoji: "📏", desc: "컨디션에 따라 피부 기복이 크며, 유분 관리가 관건인 타입입니다.", care: "기복이 심한 유분을 잡기 위해 규칙적인 압출과 진정 약침이 필요합니다." },
  ISFP: { title: "섬세한 예술가", emoji: "🎨", desc: "겉은 매끈해 보여도 속은 바짝 타들어가는 건조함을 느끼는 타입입니다.", care: "겉은 매끈해도 속당김이 심하니 심부 보습을 채워주는 게 중요해요." },
  ISFJ: { title: "사려 깊은 조력자", emoji: "🌿", desc: "내부 스트레스가 안색으로 나타나며 수분이 필요한 타입입니다.", care: "안색 개선을 위해 안면 기혈 순환을 돕는 정안침으로 생기를 더해보세요." },
  INTP: { title: "논리적인 사색가", emoji: "💡", desc: "유분은 적당하지만 컨디션에 따라 수분 장벽이 무너지기 쉬운 타입입니다.", care: "산뜻함을 유지하되, 컨디션 저하 시엔 스킨부스터로 수분 보호막을 씌워주세요." },
  INTJ: { title: "전략적인 계획가", emoji: "🎯", desc: "피부 기복이 심하며, 수분 부족과 거친 피부결이 고민인 타입입니다.", care: "갑작스러운 거칠어짐을 대비해 주기적인 약초필링으로 피부결을 정돈하세요." },
  INFP: { title: "열정적인 중재자", emoji: "🌊", desc: "속피부가 매우 예민하며 결 개선과 수분이 절실한 타입입니다.", care: "예민한 속 피부를 위해 자극이 적은 천연 모델링팩으로 부드럽게 관리하세요." },
  INFJ: { title: "선의의 옹호자", emoji: "✨", desc: "스트레스가 피부 결로 바로 이어지는 섬세한 타입입니다.", care: "감정 스트레스가 피부로 오지 않게 심신을 안정시키고 진정 케어를 병행하세요." },
  ESTP: { title: "활동적인 모험가", emoji: "🏄", desc: "외부 환경에 민감하게 반응하며 유분과 각질 관리가 필요한 타입입니다.", care: "두꺼운 각질과 유분을 정리하는 약초필링과 압출 관리가 효과적입니다." },
  ESTJ: { title: "엄격한 관리자", emoji: "📋", desc: "외부 자극으로 인해 유분형 트러블이 자주 발생하는 타입입니다.", care: "외부 자극으로 성난 유분형 트러블은 열독을 빼주는 것이 중요합니다." },
  ESFP: { title: "자유로운 영혼", emoji: "💃", desc: "외부 자극에 취약하며 장벽 강화가 최우선인 타입입니다.", care: "외부 자극에 강해지도록 피부 장벽을 강화하는 약침, 스킨부스터를 권장합니다." },
  ESFJ: { title: "사교적인 외교관", emoji: "🤝", desc: "환경 변화에 따라 수분이 급격히 부족해지는 타입입니다.", care: "환경 변화에 민감한 건조함을 잡기 위해 수분크림과 아쿠아필링이 필수예요." },
  ENTP: { title: "뜨거운 혁신가", emoji: "🔥", desc: "외부 자극으로 인한 돌발 트러블이 고민인 타입입니다.", care: "가끔 생기는 고집 센 트러블은 뿌리까지 뽑는 압출 관리로 해결하세요." },
  ENTJ: { title: "대담한 통솔자", emoji: "👑", desc: "외부 환경에 따라 피부 기복이 널뛰는 타입입니다.", care: "널뛰는 피부 컨디션을 즉각 잡아주는 진정 팩과 한약 복용이 중요합니다." },
  ENFP: { title: "재기발랄한 활동가", emoji: "🌈", desc: "미세먼지 등 외부 오염에 민감하며 결 케어가 중요한 타입입니다.", care: "미세먼지 등 외부 오염에 대비해 꼼꼼한 저자극 클렌징에 집중하세요." },
  ENFJ: { title: "정의로운 사회운동가", emoji: "☀️", desc: "외부 자극에 의해 안색이 쉽게 변하고 예민해지는 타입입니다.", care: "붉어지기 쉬운 얇은 피부를 위해 혈관을 강화하고 진정시키는 한약 복용을 추천합니다." }
};

const DIMENSION_DESCS: Record<string, string> = {
  I: "내부 변화 민감형: 몸 내부의 변화(수면, 스트레스, 생리 주기 등)에 피부 컨디션이 가장 먼저 반응합니다.",
  E: "외부 자극형: 날씨, 미세먼지, 화장품 성분 등 외부 환경 변화에 피부가 즉각적으로 반응합니다.",
  S: "유분 중심: 유분 분비가 많아 끈적임이나 번들거림이 고민이며, 산뜻한 케어를 선호합니다.",
  N: "수분 중심: 유분보다는 수분이 부족하여 피부가 당기거나 푸석해지기 쉬우며, 촉촉한 케어를 선호합니다.",
  T: "트러블 집중: 여드름, 잡티 등 눈에 보이는 피부 결점을 해결하는 것이 가장 중요한 과제입니다.",
  F: "결/감촉 집중: 피부의 당김, 결, 편안한 느낌 등 내가 직접 느끼는 컨디션을 가장 중요하게 생각합니다.",
  P: "내실형/평온함: 속피부는 민감해도 겉으로는 티가 잘 나지 않아, 내버려 두기 쉬운 타입입니다.",
  J: "표출형/들쑥날쑥: 컨디션이 조금만 나빠져도 안색이 변하거나 뒤집히는 등 티가 확 나서 세심한 관리가 필요합니다."
};
